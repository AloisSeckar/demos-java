...
This project requires **Java JDK 27** to run. Namely, [OpenJDK 27](https://jdk.java.net/27/) build is used for development and testing. For compiling, [Maven](https://maven.apache.org/index.html) is required (newest available version is advised).
...
- `--jdk=25,26,27 --code-only` - run only demos that contain an actual code from Java 25, 26, and 27
...
The most important rule: **Your code must compile and run under Java 27**. GitHub workflow is now set up to ensure this and PRs that cannot compile using `mvn clean install` will not be merged until the problems are fixed.

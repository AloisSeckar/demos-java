...
This page is co-generated by AI. Please, [report](https://github.com/AloisSeckar/demos-java/issues) issues with links and/or descriptions if needed.

## JEPMap

There is an interesting up-to-date tool that is mapping JEPs to JDKs - and also to internal projects and keywords. Be sure to visit Chris Newland's [JEPMap](https://www.chriswhocodes.com/jepmap.html) to get the full picture.

## Java JDK 27

- [JEP 523: Make G1 the Default Garbage Collector in All Environments](https://openjdk.org/jeps/523)
  - Makes the Garbage-First (G1) garbage collector the JVM's default in all environments, including constrained ones, replacing the Serial collector which was previously selected on single-CPU or low-memory systems.

- [JEP 527: Post-Quantum Hybrid Key Exchange for TLS 1.3](https://openjdk.org/jeps/527)
  - Adds three post-quantum hybrid key exchange schemes (X25519MLKEM768, SecP256r1MLKEM768, SecP384r1MLKEM1024) to the JDK's TLS 1.3 implementation, defending `javax.net.ssl` applications against future quantum computing attacks by default.

- [JEP 531: Lazy Constants (Third Preview)](https://openjdk.org/jeps/531)
  - Continues previewing the `LazyConstant` API for deferred immutability, removing the low-level `isInitialized` and `orElse` methods and adding a `Set.ofLazy(...)` factory method for lazy sets.

- [JEP 532: Primitive Types in Patterns, instanceof, and switch (Fifth Preview)](https://openjdk.org/jeps/532)
  - Continues previewing support for primitive types in pattern matching, `instanceof`, and `switch`, re-previewed without change from JDK 26.

- [JEP 533: Structured Concurrency (Seventh Preview)](https://openjdk.org/jeps/533)
  - Continues previewing structured concurrency for grouping related concurrent tasks, adding a third `Joiner`/`StructuredTaskScope` type parameter for the exception type thrown by `join()` and a new `open(UnaryOperator)` factory method.

- [JEP 534: Compact Object Headers by Default](https://openjdk.org/jeps/534)
  - Makes compact object headers, which shrink object headers from 96 to 64 bits on 64-bit architectures, the default HotSpot JVM object header layout, reducing heap size and improving performance.

- [JEP 536: JFR In-Process Data Redaction](https://openjdk.org/jeps/536)
  - Enhances JDK Flight Recorder to redact sensitive command-line arguments, environment variables, and system properties in recordings by default, preventing secrets from leaking before data leaves the process.

- [JEP 537: Vector API (Twelfth Incubator)](https://openjdk.org/jeps/537)
  - Continues incubating the Vector API for expressing SIMD-style parallel computations, re-incubated without API change and updated to use version 3.9.0 of the SLEEF library for ARM and RISC-V.

- [JEP 538: PEM Encodings of Cryptographic Objects (Third Preview)](https://openjdk.org/jeps/538)
  - Continues previewing the API for encoding and decoding cryptographic objects to and from the PEM format, changing `PEM` to an ordinary class, renaming `DEREncodable` to `BinaryEncodable`, and adding a `CryptoException` class.
